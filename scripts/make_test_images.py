"""Generate synthetic microscopy-like images for QC pipeline smoke tests."""
import os
...
def make_cells_image(n_cells=200, size=512, base_brightness=60, blur_ksize=0):
    ...
    img += np.random.normal(0, 4, size=img.shape)
...
# 5. Empty field (no cells), with clipped floating-point sensor noise
empty = np.full((512, 512), 60.0, dtype=np.float64)
empty += np.random.normal(0, 3, size=empty.shape)
