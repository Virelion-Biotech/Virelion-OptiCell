...
def bootstrap_ci(values: Sequence[float], *, n_bootstrap: int = 5000, confidence: float = 0.95, seed: int = 0) -> tuple[float, float]:
    ...
    if not 0 < confidence < 1 or n_bootstrap < 100: raise ValueError("invalid confidence or n_bootstrap")
...
def replicate_effect_summary(frame: pd.DataFrame, value_column: str, group_column: str, group_a, group_b, *, confidence: float = 0.95, n_bootstrap: int = 5000, seed: int = 0) -> dict[str, float]:
    """Summarize two replicate-level groups with effect, bootstrap CI and p-value."""
    from opticell.statistics import effect_size_mean_difference, permutation_pvalue
    a = pd.to_numeric(frame.loc[frame[group_column] == group_a, value_column], errors="coerce").to_numpy(float)
    b = pd.to_numeric(frame.loc[frame[group_column] == group_b, value_column], errors="coerce").to_numpy(float)
    ...
    return {
        "n_a": float(a.size), "n_b": float(b.size),
        ...
        "a_ci_low": low_a, "a_ci_high": high_a, "b_ci_low": low_b, "b_ci_high": high_b,
    }
...
def summarize_experiment(frame: pd.DataFrame, *, replicate_column: str, group_column: str, value_columns: Sequence[str]) -> pd.DataFrame:
    ...
    for group, group_frame in replicate.groupby(group_column, dropna=False):
        ...
        for value in value_columns:
            ...
            row[f"{value}_ci_low"] = lo; row[f"{value}_ci_high"] = hi
        rows.append(row)
