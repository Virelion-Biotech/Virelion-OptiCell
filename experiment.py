...
_WELL = re.compile(r"(?<![A-Z])([A-Ha-h])([0-9]{1,2})(?![0-9])")
...
def parse_metadata(filename: str) -> dict[str, object]:
    ...
    return {"plate": int(plate.group(1)) if plate else None, "well": f"{row}{col:02d}" if row and col else None, "well_row": row, "well_col": col, "timepoint": int(time.group(1)) if time else None}

def annotate_results(df: pd.DataFrame, filename_column: str = "filename", metadata_columns: Optional[dict[str, object]] = None) -> pd.DataFrame:
    """Add parsed plate/well/time columns, plus explicit user metadata when supplied."""
    if filename_column not in df.columns: raise ValueError(f"missing {filename_column!r}")
    result = df.copy()
    parsed = result[filename_column].map(parse_metadata).apply(pd.Series)
    for col in parsed.columns: result[col] = parsed[col]
    for key, value in (metadata_columns or {}).items(): result[key] = value
    return result
...
def compare_groups(df: pd.DataFrame, group_column: str, metric: str) -> pd.DataFrame:
    ...
    if group_column not in df or metric not in df: raise ValueError("group_column and metric must exist")
    ...
    for g in groups:
        ...
        rows.append({"group": g, "n": int(len(values)), "mean": float(values.mean()) if len(values) else np.nan, "median": float(np.median(values)) if len(values) else np.nan, "std": float(values.std(ddof=1)) if len(values) > 1 else 0.0})
...
def plate_heatmap(df: pd.DataFrame, metric: str, value: str = "mean") -> pd.DataFrame:
    ...
    if not required.issubset(df.columns): raise ValueError(f"missing columns: {sorted(required - set(df.columns))}")
    tmp = df.copy(); tmp[metric] = pd.to_numeric(tmp[metric], errors="coerce")
    ...
    for well, val in vals.items():
        if isinstance(well, str) and len(well) >= 2 and well[0] in matrix.index:
            col = int(well[1:]);
            if 1 <= col <= 12: matrix.loc[well[0], col] = float(val)
    return matrix
