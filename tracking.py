...
def link_frames(labels_by_time: list[np.ndarray], config: TrackingConfig | None = None) -> pd.DataFrame:
    ...
    if config.max_distance_px <= 0 or config.max_gap < 0:
    ...
    for t, labels in enumerate(labels_by_time):
        ...
        for prev_label, state in active.items():
            ...
            if t - last_t > config.max_gap + 1:
                continue
            for label, (x, y) in centers.items():
                d = float(np.hypot(x - last_x, y - last_y))
                if d <= config.max_distance_px * max(1.0, t - last_t):
                    candidates.append((d, prev_label, label, track_id))
        used_prev = set()
        used_new = set()
        for d, prev_label, label, track_id in sorted(candidates):
            if prev_label in used_prev or label in used_new:
                continue
            ...
            tracks.append(
                {
                    ...
                    "dy": y - py,
                    "distance_px": d,
                    "gap": t - active[prev_label][2] - 1,
                }
            )
            active.pop(prev_label, None)
            active[label] = (x, y, t, track_id)
...
def summarize_tracks(tracks: pd.DataFrame, pixel_size: float = 1.0, frame_interval: float = 1.0) -> pd.DataFrame:
    ...
    for tid, group in tracks.sort_values("frame").groupby("track_id"):
        ...
        net = float(np.hypot(g["x"].iloc[-1] - g["x"].iloc[0], g["y"].iloc[-1] - g["y"].iloc[0]))
        rows.append(
            {
                "track_id": int(tid),
                "frames": n,
                ...
                "path_length": float(step.sum() * pixel_size),
                "net_displacement": net * pixel_size,
                "mean_speed": float(step.mean() * pixel_size / frame_interval) if len(step) else 0.0,
                "net_speed": float(net * pixel_size / duration),
            }
        )
